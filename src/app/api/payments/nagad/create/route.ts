...
import {
  initNagadPayment,
  isNagadConfigured,
} from "@/infrastructure/payments/nagad";
...
export async function POST(req: Request) {
  ...
  const callbackURL = `${appUrl}/api/payments/nagad/callback`;
  ...
  try {
    ...
    return NextResponse.json({
      ...
      nagadURL: result.callBackUrl,
      mock: !isNagadConfigured(),
      amount: due,
    ...
    console.error(e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Nagad init failed" },
      { status: 502 }
    );
  }
}
